...
struct ggml_threadpool_params ggml_threadpool_params_from_cpu_params(const cpu_params & params) {
    ...
    // GGML_NUMA_REPLICATE=1 auto-config happens inside ggml_threadpool_new_impl
    // so both the common path (here) and the disposable-threadpool path (used by
    // llama-cli) get the same 192-core strict pin. Single source of truth.
