...
#include <syscall.h>
#include <numaif.h>    // For mbind() and MPOL_BIND
...
// ROBIN
// NUMA-Aware Lock-Free Work-Stealing Queue Defs
//

#define GGML_MAX_TASKS_PER_QUEUE 2048

struct ggml_task_deque {
    atomic_int top;
    atomic_int bottom;
    atomic_int tasks[GGML_MAX_TASKS_PER_QUEUE];
};

static void ggml_deque_init(struct ggml_task_deque * q) {
    atomic_store_explicit(&q->top, 0, memory_order_relaxed);
    atomic_store_explicit(&q->bottom, 0, memory_order_relaxed);
}

static void ggml_deque_push_bottom(struct ggml_task_deque * q, int task_idx) {
    int b = atomic_load_explicit(&q->bottom, memory_order_relaxed);
    atomic_store_explicit(&q->tasks[b % GGML_MAX_TASKS_PER_QUEUE], task_idx, memory_order_relaxed);
    // Ensure the task is written before the bottom pointer is updated
    atomic_thread_fence(memory_order_release);
    atomic_store_explicit(&q->bottom, b + 1, memory_order_relaxed);
}

static int ggml_deque_pop_bottom(struct ggml_task_deque * q) {
    ...
    if (t <= b) {
        // Non-empty queue
        int task = atomic_load_explicit(&q->tasks[b % GGML_MAX_TASKS_PER_QUEUE], memory_order_relaxed);
        if (t != b) {
            return task; // More than one item left
        }
...
static int ggml_deque_steal_top(struct ggml_task_deque * q) {
    int t = atomic_load_explicit(&q->top, memory_order_acquire);
    atomic_thread_fence(memory_order_seq_cst);
    int b = atomic_load_explicit(&q->bottom, memory_order_acquire);
    
    if (t < b) {
        int task = atomic_load_explicit(&q->tasks[t % GGML_MAX_TASKS_PER_QUEUE], memory_order_relaxed);
        // Attempt to increment top to steal
        if (atomic_compare_exchange_strong_explicit(&q->top, &t, t + 1, memory_order_seq_cst, memory_order_relaxed)) {
            return task;
        }
    }
    return -1; // Steal failed or empty
}
...
struct ggml_compute_state {
    ...
    // ROBIN: NUMA and Work-Stealing Tracking
    int numa_node_id; 
    struct ggml_task_deque deque;
};
...
// ROBIN, Potential need to generalize, from functions in ggml-cpu.cpp
void ggml_pin_thread_to_core(int thread_idx, struct ggml_compute_state * state) {
#if defined(__gnu_linux__)
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    
    int core_id;
    // Map Threads 0-95 to Node 0 physical cores
    if (thread_idx < 96) {
        core_id = thread_idx;
        state->numa_node_id = 0;
    } 
    // Map Threads 96-191 to Node 1 physical cores
    else if (thread_idx < 192) {
        core_id = thread_idx; // Cores 96-191 map exactly to thread_idx
        state->numa_node_id = 1;
    } 
    // Fallback/Safety (Should not be hit if running 192 threads)
    else {
        core_id = thread_idx % 192; 
        state->numa_node_id = (core_id < 96) ? 0 : 1;
    }
    ...
    if (pthread_setaffinity_np(current_thread, sizeof(cpu_set_t), &cpuset) != 0) {
        fprintf(stderr, "ggml_pin_thread_to_core: Failed to pin thread %d to core %d\n", thread_idx, core_id);
    }
#endif
}
...
// ROBIN
void * ggml_numa_alloc_on_node(size_t size, int node_id) {
#if defined(__gnu_linux__)
    // 1. Allocate raw virtual memory via mmap
    void * ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (ptr == MAP_FAILED) {
        fprintf(stderr, "ggml_numa_alloc_on_node: mmap failed for size %zu\n", size);
        return NULL;
    }

    // 2. Enforce strict physical allocation to the specific node
    unsigned long nodemask = (1UL << node_id);
    if (mbind(ptr, size, MPOL_BIND, &nodemask, sizeof(nodemask) * 8 + 1, 0) != 0) {
        fprintf(stderr, "ggml_numa_alloc_on_node: mbind failed for node %d (errno: %d)\n", node_id, errno);
        munmap(ptr, size);
        return NULL;
    }

    // 3. Immediately fault the pages to guarantee physical placement
    memset(ptr, 0, size);
...
// Returns the NUMA node that owns the given logical CPU, or -1 if unknown.
// Relies on g_state.numa.nodes[] populated by ggml_numa_init.
static int ggml_numa_node_of_cpu(int cpu) {
    if (cpu < 0) return -1;
    for (uint32_t n = 0; n < g_state.numa.n_nodes; ++n) {
        const struct ggml_numa_node * node = &g_state.numa.nodes[n];
        for (uint32_t i = 0; i < node->n_cpus; ++i) {
            if ((int)node->cpus[i] == cpu) return (int)n;
        }
    }
    return -1;
}

// Given a worker's cpumask (assumed single-bit under strict_cpu), return the NUMA node.
static int ggml_numa_node_from_cpumask(const bool * cpumask) {
    for (int i = 0; i < GGML_MAX_N_THREADS; ++i) {
        if (cpumask[i]) return ggml_numa_node_of_cpu(i);
    }
    return -1;
}
...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    // Phase 5: if this worker is pinned to NUMA node 1, read weights from the
    // mirrored Buffer B. mirror_to_node1 is a safe fallback — it returns the
    // input unchanged for pointers outside Buffer A.
    const char * src0_data = (const char *) src0->data;
#ifdef GGML_NUMA_REPLICATE
    if (params->threadpool && ggml_numa_alloc_is_ready()) {
        const int numa_node = ggml_threadpool_worker_numa_node(params->threadpool, params->ith);
        if (numa_node == 1) {
...
void ggml_compute_forward_mul_mat(
    ...
    // Phase 5: node-aware base for weights. On a Node-1 worker we read from
    // the Buffer B mirror; mirror_to_node1 safely no-ops for pointers outside
    // Buffer A, so non-replicated tensors still work.
    const char * src0_data_local = (const char *) src0->data;
#ifdef GGML_NUMA_REPLICATE
    if (params->threadpool && ggml_numa_alloc_is_ready()) {
        const int numa_node = ggml_threadpool_worker_numa_node(params->threadpool, ith);
        if (numa_node == 1) {
            src0_data_local = (const char *) ggml_numa_mirror_to_node1(src0->data);
        }
    }
#endif
...
static void set_numa_thread_affinity(int thread_n) {
    ...
    switch(g_state.numa.numa_strategy) {
        ...
        case GGML_NUMA_STRATEGY_NUMACTL:
            ...
            return;
        case GGML_NUMA_STRATEGY_REPLICATE:
            // per-worker cpumask already pinned each thread to a single physical core
            // in ggml_threadpool_new_impl; do NOT widen it back to a node-wide mask.
            return;
        default:
            return;
    }
...
// Phase 5 uses this in the mul_mat hot path to decide whether to swap to the
// Node 1 mirror. Returns -1 if unknown (threadpool not strict-pinned, or ith out of range).
int ggml_threadpool_worker_numa_node(struct ggml_threadpool * threadpool, int ith) {
    if (!threadpool || ith < 0 || ith >= threadpool->n_threads) return -1;
    return threadpool->workers[ith].numa_node_id;
}
...
static struct ggml_threadpool * ggml_threadpool_new_impl(
...
#ifdef GGML_NUMA_REPLICATE
    // If GGML_NUMA_REPLICATE=1 and the caller didn't supply a cpumask (e.g.
    // llama-cli's disposable-threadpool path), strict-pin 192 workers to
    // physical cores 0..191. Idempotent with the common.cpp auto-config.
    {
        static int announced = 0;
        const char * env = getenv("GGML_NUMA_REPLICATE");
        bool mask_set = false;
        for (int i = 0; i < GGML_MAX_N_THREADS; ++i) {
            if (tpp->cpumask[i]) { mask_set = true; break; }
        }
        if (env && env[0] == '1' && !mask_set) {
            const int n_phys = 192;
            if (n_phys <= GGML_MAX_N_THREADS) {
                memset(tpp->cpumask, 0, sizeof(tpp->cpumask));
                for (int i = 0; i < n_phys; ++i) tpp->cpumask[i] = true;
                tpp->n_threads  = n_phys;
                tpp->strict_cpu = true;
                if (!announced) {
                    fprintf(stderr, "[numa-replicate] threadpool: %d-thread strict pin across physical cores 0..%d\n",
                            n_phys, n_phys - 1);
                    announced = 1;
                }
            }
    ...
    for (int j = 0; j < tpp->n_threads; j++) {
        ...
        workers[j].numa_node_id = ggml_numa_node_from_cpumask(workers[j].cpumask);
        ggml_deque_init(&workers[j].deque);
    }
    ...
    workers[0].numa_node_id = ggml_numa_node_from_cpumask(workers[0].cpumask);
    ggml_deque_init(&workers[0].deque);
