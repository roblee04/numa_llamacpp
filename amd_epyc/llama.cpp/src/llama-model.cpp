...
static void llama_model_numa_replicate_finalize_if_enabled();
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    llama_model_numa_replicate_finalize_if_enabled();

    return true;
}

// NUMA replication finalize: copy Buffer A -> Buffer B after model load.
// Invoked from load_tensors() via the CPU backend proc-address table so this
// translation unit does not need to include ggml-cpu internal headers.
static void llama_model_numa_replicate_finalize_if_enabled() {
    auto * cpu_dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
    if (!cpu_dev) return;
    auto * cpu_reg = ggml_backend_dev_backend_reg(cpu_dev);
    if (!cpu_reg) return;
    typedef bool (*numa_is_enabled_fn)(void);
    typedef void (*numa_finalize_fn)(void);
    auto is_enabled = (numa_is_enabled_fn)
        ggml_backend_reg_get_proc_address(cpu_reg, "ggml_backend_cpu_numa_replicate_is_enabled");
    auto finalize = (numa_finalize_fn)
        ggml_backend_reg_get_proc_address(cpu_reg, "ggml_backend_cpu_numa_replicate_finalize");
    if (is_enabled && finalize && is_enabled()) {
        finalize();
    }
}
