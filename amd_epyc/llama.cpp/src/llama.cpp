...
void llama_numa_init(enum ggml_numa_strategy numa) {
    // If GGML_NUMA_REPLICATE=1 is set and the caller didn't pick a strategy,
    // auto-select REPLICATE so node enumeration runs before the threadpool is built.
    if (numa == GGML_NUMA_STRATEGY_DISABLED) {
        const char * env = std::getenv("GGML_NUMA_REPLICATE");
        if (env && env[0] == '1') {
            numa = GGML_NUMA_STRATEGY_REPLICATE;
        }
    }
    if (numa != GGML_NUMA_STRATEGY_DISABLED) {
